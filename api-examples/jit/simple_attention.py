...
class LlamaRotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, position_ids):
        ...
        inv_freq_expanded = self.inv_freq[None, :, None].to(mindspore.float32).broadcast_to((position_ids.shape[0], -1, 1))
...
def run_func(f: Callable, des:str = "function"):
    ...
    dataset = [[Tensor(np.ones((1, 512, 4096))*np.random.randn(1), mindspore.float32), None, Tensor(np.arange(512).reshape(1, -1), mindspore.float32)] for _ in range(1000)]
    out = f(*dataset[0])
